...
Safety guarantees:
  ...
  - Each book's errors are logged to pipeline/output/<slug>/run_log.json without crashing others.
...
PROFILES_DIR = PIPELINE_ROOT / "book_profiles"
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

from pipeline.mutashabihat.registry import BOOK_REGISTRY
from pipeline.mutashabihat.config import OUTPUT_DIR, DEFAULT_SOURCE_DIR
...
def _load_api_key() -> str:
    env_text = (PROJECT_ROOT / ".env").read_text(encoding="utf-8", errors="ignore")
...
def profile_missing(source_dir: Path, delay: float, api_key: str, checkpoint: PipelineCheckpoint) -> None:
    ...
    for slug in slugs_missing:
        ...
        ret = run_cmd([
            PYTHON, "-m", "pipeline.scripts.profile_books",
